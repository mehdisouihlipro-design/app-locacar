...
- [ ] **Scénario nominal (court terme)** : ouvrir le modal de détail d'un contrat `type = court` avec une ligne → cliquer "🖨 PDF" → une fenêtre d'impression s'ouvre avec le modèle officiel bilingue FR/AR (bandeau agence, identification locataire, identification véhicule remplie depuis la ligne, période/tarifs, totaux HT/TVA/TTC, montant en lettres, blocs de signature, verso conditions générales, copie client + copie agence) — une seule page (+ verso)
- [ ] **Popup bloquée** : si le navigateur bloque la popup → message d'avertissement affiché à l'utilisateur
- [ ] **Contrat long terme à 0 ou 1 ligne — véhicule vide** : un contrat `type = long` sans ligne ou avec une seule ligne → une seule page comme le court terme, mais le bloc "IV- IDENTIFICATION DU VEHICULE" (Modèle, Immatriculation, Type de carburant) est laissé vide ; le reste (identité locataire, dates, durée, montants de cette ligne) reste rempli normalement
- [ ] **Contrat long terme multi-lignes — un jeu de pages par véhicule** : un contrat `type = long` avec 2 lignes ou plus (plusieurs véhicules) → cliquer "🖨 PDF" → **une page (+ sa copie agence) par ligne de contrat**, chacune affichant SON véhicule (modèle, immatriculation, carburant) ET son propre montant (Location H.T., T.V.A., Taxe Locative calculée sur ses propres jours, Total T.T.C. de cette ligne uniquement — jamais le total agrégé du contrat sous un seul véhicule) ; une seule page de conditions générales partagée à la fin du document
