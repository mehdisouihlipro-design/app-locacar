...
router.get('/', async (req: AuthRequest, res: Response) => {
  try {
    const result = await global.db.get('/reservations?select=*&order=created_at.desc');
...
router.get('/:id', async (req: AuthRequest, res: Response) => {
  try {
    const result = await global.db.get(`/reservations?id=eq.${req.params.id}&select=*`);
...
router.put('/:id', async (req: AuthRequest, res: Response) => {
  try {
    const result = await global.db.patch(`/reservations?id=eq.${req.params.id}`, req.body, { headers: { Prefer: 'return=representation' } });
    if (!result.data || result.data.length === 0) return res.status(404).json({ success: false, message: 'Reservation not found' });
    res.json({ success: true, data: result.data[0] });
  } catch (err) {
    res.status(500).json({ success: false, error: String(err) });
...
router.delete('/:id', async (req: AuthRequest, res: Response) => {
  try {
    await global.db.delete(`/reservations?id=eq.${req.params.id}`);
